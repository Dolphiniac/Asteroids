...
bool GetColor(Rgba* color, const FbxMesh* mesh, int polygonIndex, int vertexIndex)
{
	FbxColor fbxColor;
	...
	{
		*color = Rgba((float)fbxColor.mRed, (float)fbxColor.mGreen, (float)fbxColor.mBlue, (float)fbxColor.mAlpha);
		return true;
	}
	return false;
}
...
static void ImportMesh(SceneImport* import, const FbxMesh* mesh, const Matrix44Stack& stack, const std::string& name)
{
	...
	mb->GenerateTangentSpace();
	mb->SetMat(mesh->GetNode()->GetMaterial(0)->GetName());
